...
func Test_UndirectedWithDuplicateNode(t *testing.T) {
...
func Test_DirectedWithDuplicateNode(t *testing.T) {
	gb := NewGraphBuilder(BuilderOptions{AllowDuplicateNodes: false, IsDirected: true})
	gb.AddNode(1)
	gb.AddNode(1)
	_, err := gb.Build()
	assert.ErrorIs(t, err, DuplicateNodeError{nodeID: 1})
	...
	expected_graph := rawDirectedGraph{
		Nodes: map[NodeID]*rawDirectedNode{
			1: {ID: 1, Incoming: []NodeID{}, Outgoing: []NodeID{}},
		},
		FromToEdges: map[NodeID]map[NodeID]*rawDirectedEdge{},
	}
	AssertGraphEquals(t, expected_graph, actual_graph)
...
func Test_UndirectedWithDuplicateEdge(t *testing.T) {
...
func Test_UndirectedWithRedundantEdge(t *testing.T) {
	gb := NewGraphBuilder(BuilderOptions{AllowRedundantEdges: false})
	gb.AddNode(1)
	gb.AddEdge(1, 1)
	_, err := gb.Build()
...
func Test_DirectedWithRedundantEdge(t *testing.T) {
	gb := NewGraphBuilder(BuilderOptions{AllowRedundantEdges: false, IsDirected: true})
	gb.AddNode(1)
	gb.AddEdge(1, 1)
	_, err := gb.Build()
	assert.ErrorIs(t, err, RedundantEdgeError{nodeID: 1})
	...
	expected_graph := rawDirectedGraph{
		Nodes: map[NodeID]*rawDirectedNode{
			1: {ID: 1, Incoming: []NodeID{1}, Outgoing: []NodeID{1}},
		},
		FromToEdges: map[NodeID]map[NodeID]*rawDirectedEdge{
			1: {
				1: &rawDirectedEdge{
...
func Test_UndirectedWithMultipleValuesForNode(t *testing.T) {
	gb := NewGraphBuilder()
